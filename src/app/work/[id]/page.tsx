...
type ResolvedServiceParams = {
  id: string;
  slug: string;
};
type ResolvedProjectParams = { [key: string]: string | string[] | undefined };

type ProjectDetailPageProps = {
  params: Promise<ResolvedServiceParams>;
  searchParams?: Promise<ResolvedProjectParams>;
};
...
export async function generateMetadata(
  { params: paramsPromise }: ProjectDetailPageProps
): Promise<Metadata> {
...
export default async function ProjectDetailPage({ params: paramsPromise }: DynamicPageProps) {
  const awaitedParams = await paramsPromise;
  const projectId = awaitedParams.id;
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            <Link
              href="/work"
              className="text-red-600 hover:text-red-800 group text-sm font-medium inline-flex items-center"
            >
              <span
                aria-hidden="true"
                className="transition-transform group-hover:-translate-x-1 inline-block mr-1"
              >
                ←
              </span>
              Back to Our Work
          ...
          <article>
            <header className="mb-8">
              <p className="text-base font-semibold text-red-600 uppercase tracking-wide mb-1">
                {project.category}
              </p>
